...
//     const csrftoken = getCookie('csrftoken'); // Assurez-vous que cette fonction est définie

    
//     $.ajax({
//         url: form.getAttribute('action'), // URL d'action du formulaire
//         type: 'POST',
//         contentType: false,
//         processData: false,
//         data: form_data,
...
//             if (data.success) {
//                 form.reset();
//                 closeOrderPopup(); // Fermer le popup d'origine
//                 openThankYouModal();
//             } else {
//                 // console.log('Order creation failed');
//                 openErrorModal();
...
function send_house_order(form_name) {
    ...
    $.ajax({
        ...
        success: SendHouseHandleSuccess,
        error: handleError
    });
...
// Handle successful AJAX response
function SendHouseHandleSuccess(data) {
    if (data.success) {
        form.reset();
        closeOrderPopup();
        openThankYouModal();
